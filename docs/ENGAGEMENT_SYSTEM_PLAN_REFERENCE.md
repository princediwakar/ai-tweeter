...
**Targets:** @anandmahindra, @hvgoenka, @narendramodi, @cricketaakash
...
**Two-Phase Approach:**
1. **Scout (FREE):** Counts API checks if target posted (15-min intervals)
2. **Engage (COSTS QUOTA):** Only fetch tweets when target is active

**Quota Math:**
- 8 checks/day (FREE via Counts API)
- ~15% hit rate = 1-2 retrievals/day
...
```typescript
// Rotates through targets every 15 min
const counts = await twitterClient.v2.tweetCountsRecent({
  query: `from:${target} -is:retweet -is:reply`,
...
### 3. Tweet Selection
**Opportunity Score:**
- Freshness: <10min = +50, <15min = +35, <20min = +20
- Momentum: likes/min × 10 (max +30)
- Content: Questions (+15), Business keywords (+20), Cricket keywords (+20)
- Sweet spot: 20-200 likes (+25)

### 4. Reply Generation
```typescript
const prompt = `You are replying to @${targetUsername}'s tweet.
...
URL: https://aitweeter.vercel.app/api/engage?twitter_handle=@princediwakar25
Schedule: */15 9 * * *
...
export async function GET(request: NextRequest) {
  ...
  // 5. Scout for activity (Counts API - FREE)
  // 6. If active: Fetch tweets (COSTS QUOTA)
  // 7. Select best tweet (scoring algorithm)
...
- [ ] Build all `lib/engagement/*.ts` files
...
3. **Target rotation** - Even coverage, natural variation
4. **max_results=2** - Conservative quota management
5. **Dedicated engagement persona** - business_thought_leader ≠ content personas
6. **DeepSeek for generation** - Existing integration
7. **JSON config** - No code changes to add/modify targets
8. **Account-aware endpoint** - Single `/api/engage` for all accounts
9. **Separation of concerns** - Engagement logic isolated from content generation
