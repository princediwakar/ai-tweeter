...
export default function Composer({
  ...
  return (
    <div className="fixed bottom-8 left-1/2 -translate-x-1/2 w-[min(calc(100vw-2rem),600px)] z-50 flex justify-center transition-all duration-300 ease-out">
      {/* Main Composer Pill */}
      <div
        className={`relative bg-white border shadow-sm flex items-center p-1.5 transition-all duration-500 ${
          generating
            ? "border-indigo-200 shadow-indigo-100 rounded-2xl"
            : justFinished
              ? "border-emerald-200 shadow-emerald-100 rounded-2xl"
      ...
      >
        ...
        <div className="relative flex-shrink-0" ref={dropdownRef}>
          <button
            onClick={() =>
              !generating && setShowVoiceSelector(!showVoiceSelector)
            }
            disabled={generating}
            className={`flex items-center gap-2 pl-2 pr-3 py-2 rounded-xl transition-all ${
              generating
                ? "opacity-50 cursor-not-allowed"
                : "bg-zinc-50 hover:bg-zinc-100 active:scale-[0.98]"
            }`}
          >
                    ...
                    >
                      ...
                      <div className="text-left flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-1.5 text-xs text-zinc-500 truncate mt-0.5">
                          <PlatformIcon
                            platform={
                              voiceAccount?.platform === "linkedin"
                                ? "linkedin"
                                : "twitter"
                            }
        ...
        <div className="flex-shrink-0">
          {generating ? (
            ...
            >
              <Sparkles
                className={`h-4 w-4 ${selectedVoiceId ? "text-zinc-400 group-hover:text-white transition-colors" : ""}`}
              />
              <span>Generate</span>
            </button>
          )}
