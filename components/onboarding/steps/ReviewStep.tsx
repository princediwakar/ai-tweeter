  ...
  const renderModel = (platform: string, persona: GeneratedPersona, Icon: any) => (
    <div className="p-5 border border-zinc-200 rounded-xl space-y-4 bg-zinc-50/30">
      <div className="flex items-center gap-2 pb-3 border-b border-zinc-100">
        ...
        <span className="text-sm font-semibold text-zinc-900 capitalize">{platform} Content</span>
      </div>
      
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="space-y-1">
          <label className="text-xs font-medium text-zinc-500 uppercase tracking-wider">Tone</label>
          <p className="text-sm text-zinc-900 font-medium">{persona.tone || 'Direct, helpful'}</p>
        </div>
        
        <div className="space-y-1">
          <label className="text-xs font-medium text-zinc-500 uppercase tracking-wider">Topics</label>
          <div className="flex flex-wrap gap-1 mt-1">
            {(persona.topics || ['Industry insights', 'Career growth', 'Best practices']).map(topic => (
      ...
      <div className="space-y-1 pt-2">
        ...
        <p className="text-sm text-zinc-700 leading-relaxed">
          {persona.description || 'Helping professionals grow their careers and build great products.'}
        </p>
      </div>
    </div>
  );
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="space-y-2">
        <h2 className="text-2xl font-semibold text-zinc-900 tracking-tight">Your content voice</h2>
        <p className="text-sm text-zinc-500">Here's what we'll create for you. Feel free to adjust later.</p>
      </div>
